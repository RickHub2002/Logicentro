...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      if (isValidUser) {
        const userStore = useUserStore(); // Instanciando a store do Pinia
        userStore.username = this.username; // Salvando o username na store

        this.$router.push('/pagina-inicial'); // Redireciona para a página inicial
      } else {
